...
const MAX_SANE_SELL_GAIN = 200; // ignore sell signals above this — likely a data glitch, not a real, executable price

// Quality filters — thin, low-liquidity meme coins are where manipulated/fake
// price spikes concentrate. These try to screen those out before trading them.
const MIN_LIQUIDITY_USD = 50000;   // require at least this much real pool liquidity
const MIN_VOLUME_24H_USD = 20000;  // require at least this much real 24h trading volume
const MAX_SANE_1H_CHANGE = 150;    // ignore buy signals above this — almost certainly a data glitch, not a real opportunity
...
async function fetchTokenData(addresses) {
  ...
  // Screen out thin/low-volume tokens — this is where manipulated pumps and
  // "impossible to actually sell" price spikes concentrate.
  return Object.values(byToken).filter(p => {
    const liquidity = p.liquidity?.usd || 0;
    const volume24h = p.volume?.h24 || 0;
    return liquidity >= MIN_LIQUIDITY_USD && volume24h >= MIN_VOLUME_24H_USD;
  });
}
...
async function run() {
  ...
  for (const t of tokens) {
    ...
    if (!holding) {
      ...
      const looksLikeGlitch = changeSinceBuy > MAX_SANE_SELL_GAIN;
      if (!looksLikeGlitch && (changeSinceBuy >= TAKE_PROFIT || changeSinceBuy <= STOP_LOSS)) {
